...
const OrdersList = () => {
  ...
  const fetchOrders = async () => {
    ...
    if (data) {
      // Fetch doctor names and visit_services for each order
      const doctorIds = [...new Set(data.map(o => o.assigned_doctor_id))];
      const orderIds = data.map(o => o.id);
      const [profilesRes, servicesRes] = await Promise.all([
        supabase.from('doctor_directory').select('id, full_name').in('id', doctorIds),
  ...
  const sendToDoctor = async (orderId: string) => {
    await supabase.from('visit_orders').update({ status: 'SENT_TO_DOCTOR' }).eq('id', orderId);
    toast.success("Shifokorga yuborildi");
    fetchOrders();
  };
  ...
  return (
    <DashboardLayout title="Buyurtmalar">
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        <Button onClick={() => navigate('/reception/orders/new')}>
          ...
          <TableBody>
            {orders.map(o => (
              <TableRow key={o.id}>
                ...
                <TableCell>
                  {o.status === 'NEW' && (
                    <Button size="sm" variant="outline" onClick={() => sendToDoctor(o.id)}>
